...
run_v2_loop() {
	...
	while [[ $# -gt 0 ]]; do
		...
		--completion-promise)
			if [[ -z "${2:-}" ]]; then
				print_error "--completion-promise requires a non-empty text argument"
				return 1
			fi
		...
		--max-attempts)
			if [[ -z "${2:-}" ]]; then
				print_error "--max-attempts requires a number argument"
				return 1
			fi
			if ! [[ "$2" =~ ^[0-9]+$ ]] || [[ "$2" -eq 0 ]]; then
				print_error "--max-attempts must be a positive integer, got: $2"
				return 1
			fi
			max_attempts="$2"
			shift 2
...
setup_loop() {
	...
	while [[ $# -gt 0 ]]; do
		...
		--max-iterations)
			...
			if ! [[ "$2" =~ ^[0-9]+$ ]]; then
				print_error "--max-iterations must be a non-negative integer, got: $2"
				return 1
			fi
			max_iterations="$2"
			shift 2
